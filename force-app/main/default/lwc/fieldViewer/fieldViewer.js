...
export default class FieldViewer extends LightningElement {
    ...
    @api get value() {
        ...
        if (this.datatype.isTextAreaTypes && val) {
            if (!this.isTooltip && !this.wrapText)
                val = val.replace(/\n/g, " ");
            else if (this.isTooltip && val.length > 500)
                val = val.slice(0, 500) + "...";
    ...
    get isClick2Dial() {
        return this.datatype.isClick2Dial && !this.isTooltip;
    }
    get isEmail() {
    ...
    setTypeData() {
        ...
        this.currencyDisplayAs = this.typeData.isMultiCurrency ? 'code' : 'symbol';
        ...
        this.timeZone = this.typeData.timeZone;
        this.recordId = this.typeData.recordId;
    }
    ...
    handleMouseEnter() {
        ...
        if (cellRect) {
            ...
            const detail = {
                show: true, value: this._value, currencyCode: this.currencyCode, typeData: this.typeData, cellRect: cellRect
            };
